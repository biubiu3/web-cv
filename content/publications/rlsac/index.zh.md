...
summary: "首创将鲁棒几何采样一致性建模为马尔可夫决策过程的无监督强化学习框架（ICCV 2023），彻底打破盲目随机采样的组合爆炸瓶颈，在高外点率极端场景下展现出卓越的采样效率与位姿估计精度。"
story_order: 10
homepage_order: 60
topic_keywords:
  - 强化学习
...
鲁棒估计需要从大量外点中选出一个全内点最小集。传统 RANSAC 依赖盲目随机抽样，在外点率攀升时面临指数级组合爆炸。RLSAC 独创性地将先验残差与历史轨迹作为强化学习状态，使策略网络具备主动探索与排除外点的“几何智能”。
...
污染越严重，差距越明显。此时均匀采样更容易重复探索无效组合，而 RLSAC 能把残差和历史证据带入下一轮选择，实现压倒性的效率优势。
...
去掉局部描述子后，旋转/平移 mAA 从 $0.760/0.622$ 降至 $0.702/0.568$，有力证明了几何拓扑与外观特征的深度互补。训练期的高效探索让策略学会把握全局几何分布，而在推理部署阶段则能迅速收敛于最优几何结构。

## 核心创新与范式跃迁

RLSAC 的突破在于彻底革新了计算机视觉领域四十年来对几何采样一致性“盲目试错”的传统范式：

1. **瓦解组合爆炸**：传统 RANSAC 与启发式方法面对极高外点时，计算开销呈指数级攀升。RLSAC 证明了强化学习策略能够建立“采样记忆”，在连续状态转移中主动规避劣质样本空间。
2. **零真值标注的闭环学习**：开创性地以几何求解器的共识一致性作为奖励，无需人工标注真值内点或特定最小集，实现了纯无监督的自适应进化。
3. **顶会突破与学术传承**：本研究与苏黎世联邦理工学院（ETH Zürich）Marc Pollefeys 教授团队紧密合作发表于 ICCV 2023，不仅树立了学习驱动几何采样的标杆，更为后续扩散共识范式（DiffSAC）及机器人高动态空间感知奠定了坚实的理论基石。
