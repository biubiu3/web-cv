...
每个任务都需要合适的训练数据和任务模型，直线与基础矩阵估计分别训练对应采样器。迭代生成增加 GPU 和延迟成本；训练数据、硬件或显存受限时，经典估计器仍很有吸引力。性能也会受到“有效集合”训练目标构造方式影响。

RLSAC 根据先前几何假设的奖励学习序列提议；DiffSAC 从生成角度研究哪些观测应当**一起**被选择。从强化学习到条件扩散体现了研究方法的演进，核心原则始终是让采样利用结构与反馈，把计算集中到更有价值的假设上。
