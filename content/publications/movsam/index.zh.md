...
一次语言判断可能选错物体，也可能漏掉歧义目标。MovSAM 会把当前分割结果重新放入多模态上下文，请模型再次检查。每一轮都可修改提示并更新掩码；结果稳定或达到五轮后停止。这里的“深度思考”对应一个有上限的“观察—推理—分割—复查”循环。
...
局限也由此而来：结果依赖多模态模型的推理质量；迭代闭环增加延迟，也可能继承语言模型偏差；罕见物体和异常上下文会诱发自信但错误的语义判断。在完整机器人系统中，MovSAM 可作为时间几何线索的语义补充。
