...
判断运动通常需要连续观测。丢帧或相机异常后，单帧中的类别、姿态与场景仍可用于推断运动可能性，例如骑行者或奔跑中的行人。MovSAM 研究这种时间观测缺失时的单图像分割，物理速度仍需时序测量。
...
训练数据来自人工筛选后的 DAVIS 2016、FBMS 与 SegTrackV2，共训练 100 个 epoch，使用四张 RTX 8000。人工筛选用于降低单帧语义判断与真实物理运动之间的标签歧义。
...
论文报告的单图推理时间约为 0.3 秒，适合作为恢复流程或后备感知模块；高频跟踪则需要更轻量的前馈模型。
...
MovSAM 用于缺少时序观测的场景，如掉帧后的孤立图像。模型根据类别、姿态、交互与上下文推断运动可能性。单帧无法区分某些运动状态，如静止车辆与行驶车辆，因此速度判断仍需连续观测。

局限也由此而来：结果依赖多模态模型的推理质量；迭代推理增加延迟，也可能继承语言模型偏差；罕见物体和异常上下文会诱发自信但错误的语义判断。在完整机器人系统中，MovSAM 可作为时间几何线索的语义补充。
