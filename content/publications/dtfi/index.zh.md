...
以 100 km/h 为说明性速度，由 $d=v/f$ 可算得：15 Hz 时每帧间隔内约移动 1.85 m，30 Hz 时约移动 0.93 m。这个计算解释了提高频率的动机，并非道路安全实验结论。
...
相较 PointPillars，DTFI 的 Moderate 3D AP 提高 **2.28 个百分点**，代价是运行频率下降。与表中其他融合方法相比，DTFI 的优势在于更高频率，而非最高的绝对 AP。这体现了该方法针对运行效率所作的设计取舍。

![原论文四组 KITTI 检测结果，上方为图像投影，下方为点云中的三维框。](detection-results.png "来源：原论文图 6。预测框为红色，真实标注框为绿色。")

定性结果包含密集车流、路边停车、建筑环境及远距离车辆。论文指出，远处目标有时仅有 3–5 个激光点，过度稀疏仍可能导致漏检。

## 多目标跟踪：轨迹覆盖更好，各项指标仍有取舍

原论文表 II 如下，除 ML（Mostly Lost，主要丢失轨迹比例）越低越好外，其余指标越高越好。

| 方法 | HOTA ↑ | MOTA ↑ | MOTP ↑ | MT ↑ | ML ↓ |
|---|---:|---:|---:|---:|---:|
...
DTFI 在这四个方法中获得最高 MT 和最低 ML，说明更多轨迹能被持续覆盖、完全丢失的轨迹更少。其 HOTA 高于 AB3DMOT 与 JMODT，但低于 Mono3D；MOTA 则低于其他三种方法。因此，轨迹连续性的提升不能概括为所有跟踪指标全面领先。

DTFI 估计的是三维状态，但官方 [KITTI 跟踪榜单](https://www.cvlibs.net/datasets/kitti/eval_tracking_overview.php) 使用**二维边界框重叠**计算框跟踪指标。阅读结果时需要区分模型的三维跟踪能力与榜单的评估空间。这里保留的是论文发表时的实验数据，不代表当前榜单名次。

![新出现、即将离开视野以及转弯车辆的跟踪对比。](tracking-results.png "来源：原论文图 7。上行为 AB3DMOT，下行为 DTFI；红框标出差异。")

## 消融：图像融合与多模型估计分别带来什么？

原论文表 III 单独比较是否使用图像融合，不与表 I 的主实验数值混用：

| 输入 | 3D AP：E / M / H | BEV AP：E / M / H | Hz |
|---|---|---|---:|
| 仅点云 | 79.24 / 72.17 / 66.82 | 88.31 / 85.15 / 78.26 | 50 |
| 点云＋图像 | 84.79 / 75.32 / 69.53 | 90.72 / 86.16 / 80.78 | 30 |

这一消融中，图像融合使 Moderate 3D AP 提高 **3.15 个百分点**，同时增加计算开销。原论文表 IV 进一步比较不同状态估计器：

| 估计器 | HOTA ↑ | MOTA ↑ | MOTP ↑ | MT ↑ | ML ↓ |
|---|---:|---:|---:|---:|---:|
...
从 UKF 到 IMM-UKF，HOTA 增加 4.42 个百分点；加入 PSO 调参后再增加 4.16 个百分点。最后一行在该消融的五项指标上均最好。这组结果单独呈现，不作为表 II 测试集比较的替代值。

## 结论与适用范围

DTFI 展示了如何在运行频率目标下组合图像外观、柱状三维检测和多运动模型估计。实验依据来自 KITTI；30 Hz 是指定平台上的论文报告值，不能直接推导到所有车载平台或高速公路闭环安全性能。原文指出的后续方向包括：缓解远距离稀疏点云导致的漏检，提高对细微运动方向变化的敏感性，在保持速度的同时提升检测精度，以及让检测与跟踪更紧密地联合优化。
