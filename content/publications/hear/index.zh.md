...
![长时咖啡任务同时包含演化声音与操作状态。](moka-coffee.jpg "Moka Coffee 暴露了真实长时声音与动作序列尚未解决的难点。")
...
移除 Historizer 带来的下降最大，直接支持证据缺口论证。Advancer 也改变动作行为：完整模型的低运动动作块比例为 0.15；去掉 Advancer 后为 0.33，改用回归头为 0.38。预测未来声音有助于表达进度，并减少策略停滞。
...
合成声音可支持预训练，但仍存在真实声音与合成声音之间的域差异。麦克风位置、回声、电机噪声、语言变化与端到端延迟都是部署问题。长时操作还需要可靠的失败恢复和更广泛的物理经验。
