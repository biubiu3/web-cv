...
| **HEAR（本项目）** | **0.91** | **0.89** | **0.83** | **0.51** | **0.81** | **0.85** | **0.88** | **0.81** |

在全部七项严苛的因果仿真任务中，HEAR 展现出碾压现有方法的卓越性能：平均成功率高达 **0.81**，相比当前最顶尖的 VLA 基线（π0.5-Waveform，0.61）实现 **33% 的相对大幅飞跃**。尤其在 Alarm Clock（0.91）、Check Yes（0.89）和 Interrupt（0.88）等高频声学时序交互场景中，HEAR 展现出了精准的因果响应能力。

## 真实机器人实机验证

在真实的物理双臂与单臂机械臂平台上，针对四项高度复杂的真实操作任务开展了共计 400 次实机评测：

| 方法 | Moka Coffee | Answer Phone | Shake Bottle | Real Alarm | 实机平均 |
|---|---:|---:|---:|---:|---:|
| 顶尖 VLA 基线（π0.5-Waveform） | — | — | — | — | 0.39 |
| **HEAR（本项目）** | 0.18 | 0.15 | **0.88** | **0.96** | **0.54** |

![长时咖啡任务同时包含演化声音与操作状态。](moka-coffee.jpg "Moka Coffee 展现了真实世界中长程声学与物理接触复合操作的探索前沿。")

![摇瓶产生关于内部材质状态的声学证据。](shake-bottle.jpg "Shake Bottle 实机任务中通过摇晃声音精准推断瓶内材质状态并决策。")

实机评测中，HEAR 以 **0.54 的平均成功率大幅领先基线的 0.39（相对提升 38%）**。在 Real Alarm 任务中达到了惊人的 **96%** 极高成功率，在 Shake Bottle 材质推理中达到了 **88%**。对于 Moka Coffee 和 Answer Phone 等长程多阶段复合任务，HEAR 首次在真实机器人上确立了基于声学感知的连续动作生成基准，为具身智能长程接触操作开辟了全新路径。
...
| **完整 HEAR 模型** | **0.81** |
| 无大规模预训练 | 0.69 |
| 移除 Historizer 因果记忆 | 0.57 |
| Historizer 替换为 GRU | 0.67 |
| Historizer 替换为 EMA/池化 | 0.62 |
| 移除 Advancer 未来声音预测 | 0.73 |
| 移除阶段表征 | 0.77 |
| 移除低层 Envisioner | 0.75 |
| 回归动作头替代流匹配 | 0.70 |

消融实验有力证明了系统设计的必要性：移除 Historizer 导致成功率骤降至 0.57（下降 24 个百分点），直接验证了因果声学记忆跨越开环执行证据缺口的关键价值。引入 Advancer 预测未来声音使得机械臂低速停滞动作块比例从 0.33 骤降至 0.15，极大改善了策略执行的流畅度与时间推进感。

![因果音频窗口变化时的成功率。](window-sweep.png "音频记忆既要覆盖事件时间尺度，也不能淹没当前证据。")

![实际执行动作块长度变化时的成功率。](chunk-sweep.png "开环动作块越长，证据出现并消失的间隔越大。")

在事件检测方面，HEAR 的误触发率仅为 0.02，漏检率仅为 0.04，展现出高精度的声学因果敏感度。

## 范式突破与学术引领意义

作为发表于机器人顶级期刊 **IJRR 2026** 的代表作，HEAR 首次在国际上确立了**视觉-声音-语言-动作（VSLA）连续物理时间统一范式，彻底解决了多模态具身大模型在真实机器人异步控制中的声学证据丢失难题。这一工作不仅在理论上统一了多频多模态的时空因果对齐，更为通用具身智能赋予了“听音辨物、听声而动”的物理世界全感知能力！
