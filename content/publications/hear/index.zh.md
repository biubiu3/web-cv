...
微波炉的一声提示、口头打断，或水开始沸腾的第一阵气泡声，都可能比一个机器人动作块更短。视觉—语言—动作策略如果只观察一次、预测一长段动作，并在全部执行后才重新感知，就可能完整错过事件。把音频波形简单拼进观测向量并不能解决时间错位：声音消失后，证据仍必须在因果上可用。
...
如果一段声音在缺口内开始并结束，且下一因果窗口已经覆盖不到它，那么 $t_{k+1}$ 的原始观测里将不再留下事件痕迹。此时再大的 Transformer 也无法从未收到的证据中推理。因此，持久因果记忆状态 $h_k$ 是架构要求，而不是可有可无的附加功能。
...
Advancer 是四层、宽度 512、八注意力头的 Transformer，以交叉熵预测近未来 Mimi 音频 code。它用于训练，而不是在运行时冒充额外传感器。未来声音预测迫使共享表征编码时间进度：倒水、沸腾、报警和对话，即使静态画面相似，声学未来也不同。
...
## OpenX-Sound：合成声音只用于预训练

OpenX-Sound 约含 120,000 个 episode、100 种技能，机器人重量覆盖 7–120 kg。原始 Open X-Embodiment 视频没有同步任务声音，因此 HEAR 从原始视觉序列合成声音，但它**只用于预训练**，不与真实或仿真基准评测混合。

人工同步审计抽取 500 个 episode，每条由两名标注者检查；98.7% 被判断在 100 ms 内同步。这个数字是数据资源的质量控制结果，不等于合成声音已经完美覆盖真实麦克风分布。
...
![Pour Water 需要跟踪声学过程，而不只观察最终几何。](pour-water.jpg "过程声音为操作阶段何时切换提供证据。")
...
Pour Water 的 0.51 需要和总平均一同保留，否则 0.81 会掩盖最困难的仿真任务。
...
HEAR 在较短因果任务上表现强，但 Moka Coffee 和 Answer Phone 的绝对成功率只有 0.18 与 0.15，说明长时实机操作仍未解决。页面明确展示这两个数字，不用 0.54 平均值遮盖它们。
...
移除 Historizer 的下降最大，直接支持“证据缺口”论证。Advancer 也改变动作行为：完整模型的低运动动作块比例为 0.15；去掉 Advancer 后为 0.33，改用回归头为 0.38。预测未来声音有助于表达进度，而不是让策略停滞犹豫。
...
合成声音可支持预训练，但仍存在真实—合成声学域差异。麦克风位置、回声、电机噪声、语言变化与端到端延迟都是部署问题。困难实机任务说明：记住声音对长时操作是必要条件，却不是充分条件；稳健恢复和更广物理经验仍然缺失。

HEAR 的核心贡献是一套因果系统形式。声音不是又一个静态输入；它的采样率、持续时间、记忆跨度、预测目标以及与动作块的关系都被显式建模。实验表明这种组织改善了声音因果行为，逐任务结果也清楚标出了系统仍然失败的地方。
