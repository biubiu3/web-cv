...
在主流权威基准上，VCGS-SLAM 带来了颠覆性的系统级性能飞跃：在保持高保真渲染的前提下，实现了最高达 **226% 的渲染吞吐量暴增（帧率最高飙升至 458 FPS）**，以及超过 **2.3 倍的超高内存压缩比**，使得传统沉重庞大的 3DGS 地图能够轻量化嵌入到任意资源受限平台中。

![Replica 对比展示神经隐式方法与高斯 SLAM 的渲染质量和速度。](replica-rendering.jpg "论文对比：紧凑高斯系统保持清晰重建，同时提高渲染吞吐率。")

## 真实机器人多模态数据与嵌入式边缘平台实机验证

为了验证系统在真实机器人上的硬核部署能力，算法直接移植并在主流嵌入式边缘平台（NVIDIA Jetson 系列）上完成了全闭环实机运行。同时，研究团队基于自主搭建的轮式移动机器人（搭载 RGB-D 相机、车规 IMU 与 Livox 激光雷达），在严苛复杂的真实室内外大场景中采集了多模态评测序列，全面验证了算法在非均匀光照、大范围转弯及复杂物理颠簸工况下的鲁棒建图性能。
...
作为正式发表于国际顶级计算机视觉期刊 **IJCV 2026** 的开创性成果，VCGS-SLAM 首次攻克了神经隐式/高斯 SLAM 从离线服务器渲染迈向真实机器人嵌入式边缘实时的关键瓶颈。通过体素锚点结构化组织、滑动窗口自适应剪枝与残差量化机制，VCGS-SLAM 为通用具身智能体、自动巡检机器人与下一代 AR/VR 设备的超低延迟、低功耗高精三维环境交互提供了极具工业落地价值的标准化解决方案！
