...
ERMV 将一帧引导编辑传播到完整多视角序列，保持时间与相机视角之间的一致性。
...
动作序列保持不变，视觉世界围绕它被一致地编辑。用户先编辑一张能够表达全局外观变化的帧，CLIP 视觉嵌入将它编码成 $C_{\mathrm{guide}}$。状态条件包含相机位姿、关节状态 $q$、相机位姿变化和关节变化 $\Delta q$，使生成器既知道要改成什么样，也知道传感器与机器人正在如何运动。
...
实机结果表明，加入一致的编辑轨迹能够提高 ACT 在原环境和未见杂乱环境中的成功率。下游评测补充了单帧图像质量指标。
